...
class Valid
{
     ...
     * @return Validate
     */
    public   function make($rules = [], $data=[],&$message = [], $field = [])
    {
        ...
        if(!$validate->check($data,$validate->rule)) {
            ...
            $validate->__returnData($data,$rules);
            return $data;
        }
    ...
    public function check($data, $rules = [], $scene = '')
    {
        ...
        if(is_array($data)) {
            foreach($data as $k => &$v) {
               if(is_string($v) && json_decode($v,1) ) {
                    $v = json_decode($v,1);
               }
            }
        }
    ...
    */
    ...
    protected function is($value, $rule, $data = [])
    {
        switch ($rule) {
            case 'must':
                // 必须
                $result = !empty($value) || '0' == $value || $value!='';
                break;
            ...
            case 'list':
                ...
                // 是否为数组
                $result =  !empty($value) && is_array($value) &&  $value !='';
                break;
            case 'file':
